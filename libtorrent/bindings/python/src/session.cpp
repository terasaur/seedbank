...
namespace
{
    ...
    /* seed bank -- begin mod */
    void _dict_to_string_map(string_map& dest, dict const& source)
    {
        list keys = (list)source.iterkeys();
        std::string key;
        std::string val;

        for (int i = 0; i < len(keys); ++i) {
            key = extract<std::string>(keys[i]);
            val = extract<std::string>(source[keys[i]]);
            dest[key] = val;
        }
    }

    void add_extension_with_param_map(session& s, object const& e, dict const& param_dict)
    {
        if (!extract<std::string>(e).check()) return;
        std::string name = extract<std::string>(e);
        string_map param_map;
        _dict_to_string_map(param_map, param_dict);

        if (name == "mongodb_torrent_db") {
            s.add_extension(create_mongodb_torrent_db_plugin, param_map);
        }
    }
    /* seed bank -- end mod */
...
void bind_session()
{
    ...
    class_<session, boost::noncopyable>("session", no_init)
        ...
        .def("add_extension", &add_extension_with_param_map)
#ifndef TORRENT_NO_DEPRECATE
